...
def confine_position_to_boundary(i):
    ...
    for dimension in ti.static(range(dimensions)):
        ...
        if position[dimension] <= boundary_min:
            position[dimension] = boundary_min + ti.random()
            velocities[i][dimension] *= -0.0
        elif boundary_max[dimension] <= position[dimension]:
            position[dimension] = boundary_max[dimension] - ti.random()
            velocities[i][dimension] *= -0.0
    return position
...
@ti.kernel
def calculate_pressure_force(smoothing_radius: ti.f32, target_density: ti.f32, pressure_multiplier: ti.f32, input_interaction_type: ti.i32):
    for sample_index in positions:
        ...
        for i in range(number_of_particles):
            ...
            if vector_magnitude > smoothing_radius:
                continue
            direction = vector_distance / vector_magnitude if vector_magnitude != 0 else (ti.random(), ti.random()) # direction vector
...
def simulation_step(input_interaction_type, smoothing_radius_slider, gravity_slider, target_density_slider, pressure_multiplier_slider):
    gravity_and_density(smoothing_radius_slider.value, gravity_slider.value)
    calculate_pressure_force(smoothing_radius_slider.value, target_density_slider.value, pressure_multiplier_slider.value, input_interaction_type)
...
def main():
    ...
    while gui.running and not gui.get_event(gui.ESCAPE):
        ...
        if gui.is_pressed(gui.DOWN) and play:
            ...
            input_interaction_type = 1
        elif gui.is_pressed(gui.RMB):
            input_interaction_type = 2
        elif input_interaction_type != 0:
            input_interaction_type = 0

        if len(coord) >= 2 and input_interaction_type == 0:      #sometimes coord is ['']
            for i in range(len(coord)):        
                coord[i] = int(coord[i])
            mouse_position[0], mouse_position[1] = coord[0]/1920, (1200-coord[1])/1200
        ...
        if play:
            simulation_step(input_interaction_type, smoothing_radius_slider, gravity_slider, target_density_slider, pressure_multiplier_slider)
        if show_gui:
            render(gui)
